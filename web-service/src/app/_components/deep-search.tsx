...
const defaultContents: StepContent[] = [
  {
    id: "thinking",
    title: "Thinking",
    content: (
  ...
  },
  {
    ...
    content: (
      <div className="space-y-4 mb-6">
        <h3 className="text-xl font-medium">Investigating model purpose</h3>
        <ul className="list-disc list-inside space-y-2">
          <li>
            First off, I'm trying to figure out what meta-llama/Prompt-Guard-86M
            is. It looks like a model for guarding against certain prompts,
            maybe to catch harmful or unsafe content.
          </li>
          <li>
            I'm considering whether it's designed to detect or filter out unsafe
            content, given the name. Let's check if that's accurate.
          </li>
        </ul>
  ...
  },
  {
    ...
    content: (
      <div className="space-y-4 mb-6">
        <h3 className="text-xl font-medium">Planning fine-tuning process</h3>
        <p>
          This step will outline the process for fine-tuning the model based on
          our findings...
        </p>
      </div>
    ),
...
export function DeepSearchComponent({
  ...
  initialContent = defaultContents,
  initialSearchResults = defaultSearchResults,
  title = "DeepSearch",
  subtitle = "39 Sources",
  responseTime,
}: DeepSearchProps) {
  // States
  const [isLoading, setIsLoading] = useState(!responseTime);
  const [isThinking, setIsThinking] = useState(!responseTime);
  ...
  // Dummy data setup and loading simulation
  useEffect(() => {
  ...
  // Effect for animating response time
  useEffect(() => {
    ...
    if (responseTime !== undefined) {
      setDisplayedTime(responseTime);
      setIsLoading(false);
      setIsThinking(false);
    } else {
      setDisplayedTime(0);
      intervalRef.current = setInterval(() => {
        setDisplayedTime((prevTime) => prevTime + 0.01);
      }, 10);
    }
    ...
    <div className="flex h-full max-h-xl bg-white border rounded-lg overflow-hidden shadow-sm">
      ...
      <div className="w-1/3 max-w-xs border-r bg-gray-50 p-4 flex flex-col">
        ...
        <p className="text-sm text-gray-600 mb-4">
          {responseTime !== undefined || !isLoading
            ? `Response time: ${displayedTime.toFixed(2)}s`
            : isLoading
            ? "Loading..."
            : subtitle}
        </p>
